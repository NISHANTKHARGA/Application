...
import { Search, Star, Phone, Calendar, X, SlidersHorizontal } from 'lucide-react';
...
export default function LawyersPage() {
  ...
  const fetchLawyers = useCallback(async (searchTerm = '', spec = 'All') => {
    try {
      setLoading(true);
      const params = {};
  ...
  const handleSearchImmediate = useCallback((term, spec) => {
    if (debounceRef.current) clearTimeout(debounceRef.current);
    fetchLawyers(term, spec);
  }, [fetchLawyers]);
  ...
  const handleSearchChange = useCallback((value) => {
    ...
    debounceRef.current = setTimeout(() => {
      fetchLawyers(value, selectedSpec);
    }, 300);
  }, [fetchLawyers, selectedSpec]);
  ...
  const hasActiveFilters = search || selectedSpec !== 'All';
    ...
    <UserNav>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-6 mb-6">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="flex-1 relative flex gap-2">
            ...
            <button
              onClick={() => handleSearchImmediate(search, selectedSpec)}
              disabled={loading}
          ...
          <button
            ...
            className={`md:hidden flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border transition-all text-sm ${
              showFilters || selectedSpec !== 'All'
        ...
        <div className={`${showFilters ? 'flex' : 'hidden'} md:flex items-center gap-2 overflow-x-auto pb-1 pt-3 mt-3 border-t border-gray-100`}>
      ...
      {!loading && lawyers.length > 0 && (
        <div className="flex items-center justify-between mb-4 px-1">
          <p className="text-sm text-gray-500">
            ...
            {hasActiveFilters && (
              <button
                onClick={() => { setSearch(''); setSelectedSpec('All'); fetchLawyers('', 'All'); }}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-gray-500 text-sm mb-4">
            {search
              ? `No results for "${search}"${selectedSpec !== 'All' ? ` in ${selectedSpec}` : ''}`
      ...
      {selectedLawyer && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setSelectedLawyer(null); }}
        >
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-xl">
            <div className="p-6">
              ...
              <div className="flex gap-3 pt-2 border-t border-gray-100">
                <button
                  onClick={() => setSelectedLawyer(null)}
                  className="flex-1 py-2.5 px-4 text-sm font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors"
                >
                  Close
                </button>
                {isAuthenticated && role === 'user' && (
                  <Link
                    ...
                    className="flex-1 py-2.5 px-4 text-sm font-medium text-white bg-primary hover:bg-primary-600 rounded-lg text-center transition-colors"
                  >
                    Book Appointment
                  </Link>
                )}
